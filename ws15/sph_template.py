...
def smoothing_kernel(r,h):
    ...
    if u >= 0.0 and u < 1.0:
        return alphaD*(1.0 - 1.5*u*u + 0.75*u*u*u)
    elif u < 2.0:
...
def get_accels(r,m,p,rho,av,h,n,neibs,nneibs):
    ...
    for i in range(n):
        for j in range(nneibs[i]):
            jj = neibs[i,j]
            rij = r[i]-r[jj]
            accels[i] -= m[jj]*(p[i]/(rho[i]*rho[i])*dsmoothing_kernel_rij(rij,h) + p[jj]/(rho[jj]*rho[jj])*dsmoothing_kernel_rij(rij,h))
...
def get_energyRHS(r,m,p,rho,v,av,h,n,neibs,nneibs):
    ...
    for i in range(n):
        for j in range(nneibs[i]):
            ...
            rij = r[i]-r[jj]
            vij = v[i]-v[jj]
            epsrhs[i] += 0.5*m[jj]*(p[i]/(rho[i]*rho[i]) + p[jj]/(rho[jj]*rho[jj]) + av[i,jj])*vij*dsmoothing_kernel_rij(rij,h)
    return epsrhs
...
for nt in range(1,ntmax):
    ...
    if nt == 1 or nt % 5 == 0:
        ax = mpl.gca()
        mpl.clf()
        p1, =mpl.plot(r[nghost:len(rho)-nghost],rho[nghost:len(rho)-nghost],"+")
        mpl.xlabel("Radius (cm)")
        mpl.ylabel("Density (g/cm3)")
        Time = "Time: " + str(time)
        mpl.text(0.9, 0.9, Time, horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
        mpl.draw()
