...
def reconstruct(hyd,gamma,type):
    ...
    if(type=='pc'):
        ...
        hyd.rhop = hyd.rho.copy()
        hyd.rhom = hyd.rho.copy()
        hyd.epsp = hyd.eps.copy()
        hyd.epsm = hyd.eps.copy()
        hyd.velp = hyd.vel.copy()
        hyd.velm = hyd.vel.copy()
...
def hlle(hyd):
    ...
    # right are qm[:,i+1]
    #for i in range(1,hyd.n-2):
    fluxl[0,:] = hyd.rhop.copy() * hyd.velp.copy()
    fluxl[1,:] = hyd.rhop.copy() * hyd.velp.copy() * hyd.velp.copy() * hyd.pressp.copy()
    fluxl[2,:] = (hyd.rhop.copy()*hyd.epsp.copy() + 0.5*hyd.rhop.copy()*hyd.velp.copy()*hyd.velp.copy() + hyd.pressp.copy())*hyd.velp.copy()

    fluxl[0,:] = hyd.rhom.copy() * hyd.velm.copy()
    fluxl[1,:] = hyd.rhom.copy() * hyd.velm.copy() * hyd.velm.copy() * hyd.pressm.copy()
    fluxl[2,:] = (hyd.rhom.copy()*hyd.epsm.copy() + 0.5*hyd.rhom.copy()*hyd.velm.copy()*hyd.velm.copy() + hyd.pressm.copy())*hyd.velm.copy()

        #fluxl[0,i] = hyd.rhop[i] * hyd.velp[i]
        #fluxl[1,i] = hyd.rhop[i] * hyd.velp[i] * hyd.velp[i] * hyd.pressp[i]
        #fluxl[2,i] = (hyd.rhop[i]*hyd.epsp[i] + 0.5*hyd.rhop[i]*hyd.velp[i]*hyd.velp[i] + hyd.pressp[i])*hyd.velp[i]

        #fluxr[0,i] = hyd.rhom[i] * hyd.velm[i]
        #fluxr[1,i] = hyd.rhom[i] * hyd.velm[i] * hyd.velm[i] * hyd.pressm[i]
        #fluxr[2,i] = (hyd.rhom[i]*hyd.epsm[i] + 0.5*hyd.rhom[i]*hyd.velm[i]*hyd.velm[i] + hyd.pressm[i])*hyd.velm[i]
...
numpy_time = timeit("hlle2()", number=10, setup="from hydro_1D_FV_np import hlle2")
loop_time = timeit("hlle2()", number=10, setup="from hydro_1D_FV_template import hlle2")
